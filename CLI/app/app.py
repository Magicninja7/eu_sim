...
from chain_of_events.coe_compiler import compute_ready_events
...
def reset_game():
    global policy_state, stats_state, notification_sequence
    global possible_ev, las_increment, game_over, collapsed_stats
    ...
    new_close = cls_elections(Event, Transition, stats_state)
    new_polar = polarisat(Event, Transition, stats_state)
    new_terr1 = terrorist_isis_1(Event, Transition, stats_state)
    new_terr_att = terrorist_att_isis_1(Event, Transition, stats_state)

    EVENTS.clear()
    EVENTS.extend([new_polar, new_terr1, new_terr_att, new_close])

    NAMES.clear()
    NAMES.update({
        'POLARISATION': new_polar,
        'TERRORIST_ISIS_1': new_terr1,
        'TERRORIST_ATT_ISIS_1': new_terr_att,
        'CLOSE_ELECTIONS': new_close,
    })

    CURR_OP.clear()
    CURR_OP.update({f'EVENTS_{name}': 0 for name in NAMES})
    NXT_EVENT.clear()
    NXT_EVENT.update({f'EVENTS_{name}': None for name in NAMES})
months = {
    1: 31,
...
def create_app() -> Flask:
    ...
    def select_next_event() -> dict[str, Any] | None:
        possible = compute_ready_events(EVENTS, CURR_OP, NXT_EVENT, NAMES)
        if not possible:
            return None
    ...
    def status_payload():
        ...
        return {
            ...
            "tax_l": policy_state.taxation['low'],
            "tax_m": policy_state.taxation['medium'],
            "tax_h": policy_state.taxation['high'],
            "stats": generalise(stats_state),
            "game_over": is_over,
            "collapsed": collapsed,
        }
    ...
    def api_choose():
        ...
        is_over, collapsed = check_game_over()
        if is_over:
            return jsonify({"ok": True, "done": True, "message": "Game over.", "status": status_payload()})

        state["current"] = None
        next_current = ensure_current_event()
        if next_current is None:
            return jsonify({"ok": True, "done": True, "message": state.get("last_message") or "No more events available.", "status": status_payload()})
        event = next_current["event"]
        decisions = [{"id": idx, "label": choice.label} for idx, choice in enumerate(next_current["choices"])]
        return jsonify({"ok": True, "done": False, "description": event.description, "decisions": decisions, "status": status_payload()})
